...
namespace poly_traj
{
  ...
  public:
    ...
    // GaaiLam
    inline bool project_pt(const Eigen::Vector3d& pt, double& tt, Eigen::Vector3d& pro_pt)
    {
      // 2*(p-p0)^T * \dot{p} = 0
    ...
    inline bool intersection_plane(const Eigen::Vector3d p, const Eigen::Vector3d v, double& tt, Eigen::Vector3d& pt) const
    {
      // (pt - p)^T * v = 0
  ...
  public:
    ...
    Trajectory(const std::vector<double>& durs, const std::vector<CoefficientMat>& cMats)
    {
    ...
    inline bool project_pt(const Eigen::Vector3d& pt, int& ii, double& tt, Eigen::Vector3d& pro_pt)
    {
    ...
    inline bool intersection_plane(const Eigen::Vector3d p, const Eigen::Vector3d v, int& ii, double& tt, Eigen::Vector3d& pt)
    {
      for (int i = 0; i < getPieceNum(); ++i)
      {
  ...
  private:
    ...
    inline void addPropCtoT(const Eigen::MatrixXd& adjGdC, EIGENVEC& gdT) const
    {
      ...
      for (int i = 0; i < N - 1; i++)
      {
        negVel = -(b.row(i * 6 + 1) + 2.0 * T1(i) * b.row(i * 6 + 2) + 3.0 * T2(i) * b.row(i * 6 + 3) + 4.0 * T3(i) * b.row(i * 6 + 4) +
                   5.0 * T4(i) * b.row(i * 6 + 5));
        negAcc = -(2.0 * b.row(i * 6 + 2) + 6.0 * T1(i) * b.row(i * 6 + 3) + 12.0 * T2(i) * b.row(i * 6 + 4) + 20.0 * T3(i) * b.row(i * 6 + 5));
        negJer = -(6.0 * b.row(i * 6 + 3) + 24.0 * T1(i) * b.row(i * 6 + 4) + 60.0 * T2(i) * b.row(i * 6 + 5));
      ...
      negVel = -(b.row(6 * N - 5) + 2.0 * T1(N - 1) * b.row(6 * N - 4) + 3.0 * T2(N - 1) * b.row(6 * N - 3) + 4.0 * T3(N - 1) * b.row(6 * N - 2) +
                 5.0 * T4(N - 1) * b.row(6 * N - 1));
      negAcc =
          -(2.0 * b.row(6 * N - 4) + 6.0 * T1(N - 1) * b.row(6 * N - 3) + 12.0 * T2(N - 1) * b.row(6 * N - 2) + 20.0 * T3(N - 1) * b.row(6 * N - 1));
      negJer = -(6.0 * b.row(6 * N - 3) + 24.0 * T1(N - 1) * b.row(6 * N - 2) + 60.0 * T2(N - 1) * b.row(6 * N - 1));
    ...
    template <typename EIGENVEC>
    inline void addTimeIntPenalty(const Eigen::VectorXi cons, const Eigen::VectorXi& idxHs, const std::vector<Eigen::MatrixXd>& cfgHs,
                                  const double vmax, const double amax, const Eigen::Vector3d ci, double& cost, EIGENVEC& gdT,
        ...
        for (int j = 0; j < innerLoop; j++)
        {
          ...
          for (int k = 0; k < K; k++)
          {
            ...
            if (violaPos > 0.0)
            {
              ...
              gdT(i) += omg * (ci(0) * violaPosPenaD * alpha * outerNormal.dot(vel) * step + ci(0) * violaPosPena / cons(i));
          ...
          if (violaVel > 0.0)
          {
            ...
            gdT(i) += omg * (ci(1) * violaVelPenaD * gradViolaVt * step + ci(1) * violaVelPena / cons(i));
          ...
          if (violaAcc > 0.0)
          {
            ...
            gdT(i) += omg * (ci(2) * violaAccPenaD * gradViolaAt * step + ci(2) * violaAccPena / cons(i));
  ...
  public:
    ...
    inline void generate(const Eigen::MatrixXd& inPs, const Eigen::VectorXd& ts)
    {
      if (inPs.cols() == 0)
      {
        ...
        coeffMatReversed.col(5) = 0.5 * (tailPVA.col(2) - headPVA.col(2)) * t3_inv - 3.0 * (headPVA.col(1) + tailPVA.col(1)) * t4_inv +
                                  6.0 * (tailPVA.col(0) - headPVA.col(0)) * t5_inv;
        coeffMatReversed.col(4) = (-tailPVA.col(2) + 1.5 * headPVA.col(2)) * t2_inv + (8.0 * headPVA.col(1) + 7.0 * tailPVA.col(1)) * t3_inv +
                                  15.0 * (-tailPVA.col(0) + headPVA.col(0)) * t4_inv;
        coeffMatReversed.col(3) = (0.5 * tailPVA.col(2) - 1.5 * headPVA.col(2)) * t1_inv - (6.0 * headPVA.col(1) + 4.0 * tailPVA.col(1)) * t2_inv +
    ...
    inline double getTrajJerkCost() const
    {
      ...
      for (int i = 0; i < N; i++)
      {
        objective += 36.0 * b.row(6 * i + 3).squaredNorm() * T1(i) + 144.0 * b.row(6 * i + 4).dot(b.row(6 * i + 3)) * T2(i) +
    ...
    template <typename EIGENVEC, typename EIGENMAT>
    inline void evalTrajCostGrad(const Eigen::VectorXi& cons, const Eigen::VectorXi& idxHs, const std::vector<Eigen::MatrixXd>& cfgHs,
                                 const double& vmax, const double& amax, const Eigen::Vector3d& ci, double& cost, EIGENVEC& gdT, EIGENMAT& gdInPs)
    {
      gdT.setZero();
      gdInPs.setZero();
      gdC.setZero();
