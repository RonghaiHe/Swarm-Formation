...
private:
  ...
  int STEP_2()
  {
        ...
        for (int col = 0; col < n_col_; col++)
        {
          ...
          {                                 //find uncovered zero
            ...
            {                         //if starred zero
              ...
              {
                if (star_matrix_[row][column])
                { //uncover col containing star zero
    ...
    { //save the smallest uncovered element, e_min
  ...
  int STEP_3()
  {
    ...
    while (col_starred(Z0.second))
    { //if column of Z0 has a starred zero Z1
      Z1 = get_star_zero(Z0.second);
      S.emplace_back(Z1); // insert Z1 to S

      for (int col = 0; col < n_col_; col++)
      {
        if (prime_matrix_[Z1.first][col])
        { //Replace Z0 with the primed zero in the row of Z1
