...
namespace lbfgs
{
  ...
  /**
     ...
     */
  typedef int (*lbfgs_progress_t)(void* instance, const double* x, const double* g, const double fx, const double xnorm, const double gnorm,
...
#define QUARD_MINIMIZER2_LBFGS(qm, u, du, v, dv)                                                                                                     \
  ...
  /**
     ...
     */
  inline int update_trial_interval(double* x, double* fx, double* dx, double* y, double* fy, double* dy, double* t, double* ft, double* dt,
  ...
  inline int line_search_morethuente(int n, double* x, double* f, double* g, double* s, double* stp, const double* xp, const double* gp,
                                     const double* stpmin, const double* stpmax, callback_data_t* cd, const lbfgs_parameter_t* param)
  {
    int count = 0;
    ...
    for (;;)
    {
      ...
      *f = cd->proc_evaluate(cd->instance, x, g, cd->n);
      ...
      /*
            ...
            */
      if (stage1 && *f <= ftest1 && (param->f_dec_coeff <= param->s_curv_coeff ? param->f_dec_coeff : param->s_curv_coeff) * dginit <= dg)
      {
        stage1 = 0;
      }
  ...
  /**
     ...
     */
  inline int lbfgs_optimize(int n, double* x, double* ptr_fx, lbfgs_evaluate_t proc_evaluate, lbfgs_stepbound_t proc_stepbound,
