...
inline int
GridMap::toAddress(const Eigen::Vector3i& id)
{
  return id(0) * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + id(1) * mp_.map_voxel_num_(2) + id(2);
}

inline int
GridMap::toAddress(int& x, int& y, int& z)
{
  return x * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + y * mp_.map_voxel_num_(2) + z;
}

inline void
GridMap::boundIndex(Eigen::Vector3i& id)
{
  Eigen::Vector3i id1;
...
inline bool
GridMap::isUnknown(const Eigen::Vector3i& id)
{
...
inline bool
GridMap::isUnknown(const Eigen::Vector3d& pos)
{
  Eigen::Vector3i idc;
  posToIndex(pos, idc);
...
inline bool
GridMap::isKnownFree(const Eigen::Vector3i& id)
{
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
...
inline bool
GridMap::isKnownOccupied(const Eigen::Vector3i& id)
{
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
  int adr = toAddress(id1);
...
inline void
GridMap::setOccupied(Eigen::Vector3d pos)
{
  if (!isInMap(pos))
    return;
...
inline void
GridMap::setOccupancy(Eigen::Vector3d pos, double occ)
{
  if (occ != 1 && occ != 0)
  {
...
inline int
GridMap::getOccupancy(Eigen::Vector3d pos)
{
  if (!isInMap(pos))
    return -1;
...
inline int
GridMap::getInflateOccupancy(Eigen::Vector3d pos)
{
  if (!isInMap(pos))
    return -1;
...
inline int
GridMap::getOccupancy(Eigen::Vector3i id)
{
...
inline bool
GridMap::isInMap(const Eigen::Vector3d& pos)
{
...
inline bool
GridMap::isInMap(const Eigen::Vector3i& idx)
{
...
inline void
GridMap::posToIndex(const Eigen::Vector3d& pos, Eigen::Vector3i& id)
{
  for (int i = 0; i < 3; ++i)
...
inline void
GridMap::indexToPos(const Eigen::Vector3i& id, Eigen::Vector3d& pos)
{
  for (int i = 0; i < 3; ++i)
...
inline void
GridMap::inflatePoint(const Eigen::Vector3i& pt, int step, vector<Eigen::Vector3i>& pts)
{
  int num = 0;
...
inline double
GridMap::getDistance(const Eigen::Vector3d& pos)
{
  Eigen::Vector3i id;
  posToIndex(pos, id);
  boundIndex(id);
...
inline double
GridMap::getDistance(const Eigen::Vector3i& id)
{
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
